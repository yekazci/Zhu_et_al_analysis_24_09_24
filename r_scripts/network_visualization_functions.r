...
vis_network_w_degree_thresholded_vertices_OLD_FUNCTION_w_GGNET2 <- function(meta_DF, score_cut = NULL, 
                          target_genes = NULL,
  ...
  # Extract the names of the vertices (nodes) in the network.
  vertex_names <- get.vertex.attribute(x = net, attrname = 'vertex.names')
  ...
  # Set the class attribute of each vertex to the determined identity ('target', 'TF', or 'target-TF').
  set.vertex.attribute(x = net, attrname = 'class', value = identity)
...
# are also Transcription factors, we applied this above.
  ...
  # Clean up the vertex names by removing the ".target-TF" suffix.
  set.vertex.attribute(net, "vertex.names", gsub(pattern = ".target-TF", replacement = "", x = get.vertex.attribute(net, "vertex.names"), fixed = TRUE))
...
set.vertex.attribute(x = net, attrname = 'for_legend', value = for_legend)
...
# Create a directed network object from the 'tf' and 'target' columns, interpreted as an edge list.
  ...
  if (!is.null(degree)) {
     ...
     p1 <- YOUR_FAVORITE_VIS_FUNCTION(meta_DF = meta_DF, 
                                  score_cut = score_cut, 
